...
output reg                  sq_o_load_resp_wait_1,
    ...
    localparam [`ROB_IDX_W:0] SQ_CAPACITY = `ROB_SIZE;
...
wire [`ROB_IDX_W-1:0] alloc2_ptr =
    ...
    task build_load_response;
        ...
        output                      resp_wait;
        output     [`DWIDTH-1:0]    resp_forward_data;

        reg found_match;
        reg found_unresolved;
        reg found_partial;
        reg [`ROB_IDX_W-1:0] best_dist;
        reg [`ROB_IDX_W-1:0] cur_dist;
        reg [`DWIDTH-1:0] best_data;
        begin
            q_i = 0;
            cur_dist = {`ROB_IDX_W{1'b0}};
            resp_valid = query_valid;
            resp_ptr = query_ptr;
            resp_read_mem = 1'b0;
            resp_forward_valid = 1'b0;
            resp_wait = 1'b0;
            resp_forward_data = {`DWIDTH{1'b0}};

            found_match = 1'b0;
            found_unresolved = 1'b0;
            found_partial = 1'b0;
            best_dist = {`ROB_IDX_W{1'b0}};
            best_data = {`DWIDTH{1'b0}};

            if (query_valid) begin
                for (q_i = 0; q_i < `ROB_SIZE; q_i = q_i + 1) begin
                    if (query_in_older_range(q_i[`ROB_IDX_W-1:0],
                        ...
                        if (!ent_filled[q_i]) begin
                            ...
                            if ((ent_mask[q_i] & query_mask) == query_mask) begin
                                cur_dist = q_i[`ROB_IDX_W-1:0] - head_ptr;
                                if (!found_match || (cur_dist >= best_dist)) begin
                                    found_match = 1'b1;
                                    best_dist = cur_dist;
                                    best_data = ent_data[q_i];
                                end
                            end
                ...
                if (found_unresolved || found_partial) begin
                    resp_wait = 1'b1;
                end
                else if (found_match) begin
                    resp_forward_valid = 1'b1;
                    resp_forward_data = best_data;
                end
                else begin
                    resp_read_mem = 1'b1;
                end
    ...
    always @(*) begin
        ...
        build_load_response(sq_i_load_query_valid_1,
                            ...
                            sq_o_load_resp_valid_1,
                            sq_o_load_resp_ptr_1,
                            sq_o_load_resp_read_mem_1,
                            sq_o_load_resp_forward_valid_1,
                            sq_o_load_resp_wait_1,
        ...
        build_load_response(sq_i_load_query_valid_2,
                            ...
                            sq_o_load_resp_valid_2,
                            sq_o_load_resp_ptr_2,
                            sq_o_load_resp_read_mem_2,
                            sq_o_load_resp_forward_valid_2,
                            sq_o_load_resp_wait_2,
