`timescale 1ns/1ps
// Reservation Station variant with a registered issue-index stage.
//
// Cycle N:
//   - Build ready_vec.
//   - Select up to two ready RS entry indices.
//   - Latch the selected indices into issueq_*.
//
// Cycle N+1:
//   - Use the registered issueq indices to mux the full payload to rs_o_*.
//   - Downstream accept clears the RS entries.
//
// this keeps the full-payload rs architecture, but removes the direct
// ready_vec -> priority select -> wide payload mux path from one cycle.
module reservation_station (
    ...
    function dep_rs_on_prd;
        ...
        input [`OPCODE_WIDTH-1:0] opcode;
        begin
            dep_rs_on_prd = opcode_has_rs(opcode) && (prs == prd);
        end
    endfunction
    ...
    function dep_rt_on_prd;
        ...
        input [`OPCODE_WIDTH-1:0] opcode;
        begin
            dep_rt_on_prd = opcode_has_rt(opcode) && (prt == prd);
        end
    endfunction
    ...
    always @(*) begin
        ...
        for (idx_c = 0; idx_c < `RS_SIZE; idx_c = idx_c + 1) begin
            ...
            // original conservative ready check:
            // ready_vec[idx_c] = ent_valid[idx_c] &&
            //                    !ent_issued[idx_c] &&
            //                    (!ent_has_rs[idx_c] || ent_rs_ready[idx_c]) &&
            //                    (!ent_has_rt[idx_c] || ent_rt_ready[idx_c]);
            //
            // execute-only wakeup-to-select bypass: alu/execute results are
            // selectable in the same cycle. memory/load wakeups still wait for
            // ent_*_ready to be registered, avoiding unsafe load-use issue.
            ready_vec[idx_c] = ent_valid[idx_c] &&
                               !ent_issued[idx_c] &&
