...
#ifndef BEEBS_CRC_EXPECT
#define BEEBS_CRC_EXPECT 1207487004UL
#endif
...
void beebs_entry(void)
{
    ...
    for (i = 0; i < BEEBS_CRC_ITERS; ++i) {
        int r;
        seed = (seed * 1103515245L + 12345) & ((1UL << 31) - 1);
        r = (int)(seed >> 16);
        oldcrc32 = crc_32_tab[(oldcrc32 ^ ((BYTE)r)) & 0xff] ^ (oldcrc32 >> 8);
    }

    result = ~oldcrc32;
#ifdef BEEBS_SKIP_CHECK
    status = 1;
#else
