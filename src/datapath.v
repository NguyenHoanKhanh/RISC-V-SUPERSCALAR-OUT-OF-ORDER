...
`include "ROB.v"
`include "execute_stage_1cycle.v"
...
//     assign rs_dbg_entry_idx = u_rs.issue1_idx;
...
//     wire lane1_allows_lane2_pre = (!ds1_rs_o_ce) || (!ds_req_1) || pre_ok_1;
//     wire pre_req_2 = ds_req_2 && ru_rs_can_take_2 && lane1_allows_lane2_pre;
...
//     // -------------------------------------------------------------------------
//     wire dec_consume_1 = ds1_rs_o_ce && ((!ds_req_1) || ren_fire_1);
//     wire dec_consume_2 = ds2_rs_o_ce && ((!ds_req_2) || ren_fire_2);
//     wire dec_lane_done_1 = (!ds1_rs_o_ce) || dec_consume_1;
//     wire dec_lane_done_2 = (!ds2_rs_o_ce) || dec_consume_2;
//     wire dec_pair_done = dec_lane_done_1 && dec_lane_done_2;
//     wire decode_hold = !dec_pair_done;
//     wire fe_lane_done_1 = dec_lane_done_1;
//     wire fe_lane_done_2 = dec_lane_done_2;
//     wire ds_rs_any_valid = ds1_rs_o_ce || ds2_rs_o_ce;
//     wire fe_stall = decode_hold;
...
//             .rs_i_rob_head_tag(rob_o_commit_tag_1),
//             .rs_o_full(rs_o_full),
...
//             .dbg_entry_idx(rs_dbg_entry_idx),
//             .dbg_ent_valid(rs_dbg_ent_valid),
//             .dbg_ent_prs(rs_dbg_ent_prs),
//             .dbg_ent_prt(rs_dbg_ent_prt),
//             .dbg_ent_rs_ready(rs_dbg_ent_rs_ready),
//             .dbg_ent_rt_ready(rs_dbg_ent_rt_ready),
//             .dbg_ent_vrs(rs_dbg_ent_vrs),
//             .dbg_ent_vrt(rs_dbg_ent_vrt),
//             .dbg_ready_vec(rs_dbg_ready_vec),
// 	        .rs_o_stall_2()
// 	    );
...
//     treat_load u_tl1 (
//         .tl_i_load_addr(lq_o_complete_addr_1),
...
//     treat_load u_tl2 (
//         .tl_i_load_addr(lq_o_complete_addr_2),
...
//     reg [`ROB_IDX_W : 0] rob_used_shadow;
//     // wire [`ROB_IDX_W : 0] rob_reserved_shadow = rob_used_shadow + ru_rs_pending_allocs;
//     wire [`ROB_IDX_W : 0] rob_free_shadow = `ROB_SIZE - rob_used_shadow;
...
//     wire ru_rs_lane2_older_pending = ru_rs_lane2_older_q || (ru_rs_valid_2 && !ru_rs_valid_1);
...
//     wire ru_rs_can_take_2 = (~ru_rs_valid_2) ||
//                             (ru_rs_dispatch_fire_2 && !ru_rs_lane2_older_pending);

//     wire pre_req_1 = ds_req_1 && ru_rs_can_take_1 && !ru_rs_lane2_older_pending;
...
//     // Registered RS free-slot credit breaks the RS -> dispatch combinational feedback
//     // while still allowing ru_rs to refill when its current entry dispatches.
//     reg [`ROB_IDX_W : 0] rs_free_credit_q;
//     wire [`ROB_IDX_W : 0] rs_size_ext = `RS_SIZE;
//     wire [`ROB_IDX_W : 0] rs_issue_count =
//         {{`ROB_IDX_W{1'b0}}, rs_issue_accept_1} +
//         {{`ROB_IDX_W{1'b0}}, rs_issue_accept_2};
//     wire [`ROB_IDX_W : 0] rs_dispatch_count =
//         {{`ROB_IDX_W{1'b0}}, ru_rs_dispatch_fire_1} +
//         {{`ROB_IDX_W{1'b0}}, ru_rs_dispatch_fire_2};
//     wire [`ROB_IDX_W : 0] rs_free_after_issue = rs_free_credit_q + rs_issue_count;
//     wire [`ROB_IDX_W : 0] rs_free_after_issue_sat =
//         (rs_free_after_issue > rs_size_ext) ? rs_size_ext : rs_free_after_issue;
//     wire [`ROB_IDX_W : 0] rs_free_next =
//         (rs_free_after_issue_sat > rs_dispatch_count) ?
//         (rs_free_after_issue_sat - rs_dispatch_count) : {(`ROB_IDX_W + 1){1'b0}};
//     wire rs_credit_can_alloc_1 = (rs_free_credit_q >= {{`ROB_IDX_W{1'b0}}, 1'b1});
//     wire rs_credit_can_alloc_2 = (rs_free_credit_q >= {{(`ROB_IDX_W - 1){1'b0}}, 2'd2});
...
//     wire dispatch_can_take_1 =
//         rob_o_can_alloc_1 & rs_credit_can_alloc_1 & sq_lane1_ok & lq_lane1_ok;
//     wire dispatch_can_take_2_pair =
//         rob_o_can_alloc_2 & rs_credit_can_alloc_2 & sq_pair_ok & lq_pair_ok;
//     wire dispatch_can_take_2_solo =
//         rob_o_can_alloc_1 & rs_credit_can_alloc_1 & sq_lane2_solo_ok & lq_lane2_solo_ok;
...
//     wire cpl1_from_pipe1 = mwb_valid_1;
//     wire cpl1_from_pipe2 = !mwb_valid_1 && mwb_valid_2;
//     wire cpl1_from_lq1 = !mwb_valid_1 && !mwb_valid_2 && lq_mwb_complete_valid_1;
//     wire cpl1_from_lq2 = !mwb_valid_1 && !mwb_valid_2 &&
//                          !lq_mwb_complete_valid_1 && lq_mwb_complete_valid_2;

//     wire one_pipe_cpl = mwb_valid_1 ^ mwb_valid_2;
//     wire cpl2_from_pipe2 = mwb_valid_1 && mwb_valid_2;
//     wire cpl2_from_lq1 = one_pipe_cpl && lq_mwb_complete_valid_1;
//     wire cpl2_from_lq2 = (one_pipe_cpl && !lq_mwb_complete_valid_1 &&
//                           lq_mwb_complete_valid_2) ||
//                          (!mwb_valid_1 && !mwb_valid_2 &&
//                           lq_mwb_complete_valid_1 && lq_mwb_complete_valid_2);

//     wire lq_mwb_accept_1 = cpl1_from_lq1 || cpl2_from_lq1;
//     wire lq_mwb_can_load_1 = !lq_mwb_complete_valid_1 || lq_mwb_accept_1;
...
//         cpl1_from_pipe1 ? mwb_rob_idx_1 :
//         cpl1_from_pipe2 ? mwb_rob_idx_2 :
//         cpl1_from_lq1   ? lq_mwb_complete_rob_tag_1 :
//         cpl1_from_lq2   ? lq_mwb_complete_rob_tag_2 : {`ROB_IDX_W{1'b0}};

//     wire [`ROB_IDX_W - 1 : 0] cpl_tag_2 =
//         cpl2_from_pipe2 ? mwb_rob_idx_2 :
//         cpl2_from_lq1   ? lq_mwb_complete_rob_tag_1 :
//         cpl2_from_lq2   ? lq_mwb_complete_rob_tag_2 : {`ROB_IDX_W{1'b0}};

//     wire [`RAT_SIZE - 1 : 0] cpl_prd_1 =
//         cpl1_from_pipe1 ? mwb_tag_1 :
//         cpl1_from_pipe2 ? mwb_tag_2 :
//         cpl1_from_lq1   ? lq_mwb_complete_prd_1 :
//         cpl1_from_lq2   ? lq_mwb_complete_prd_2 : {`RAT_SIZE{1'b0}};

//     wire [`RAT_SIZE - 1 : 0] cpl_prd_2 =
//         cpl2_from_pipe2 ? mwb_tag_2 :
//         cpl2_from_lq1   ? lq_mwb_complete_prd_1 :
//         cpl2_from_lq2   ? lq_mwb_complete_prd_2 : {`RAT_SIZE{1'b0}};
...
module datapath (
    ...
    // Keep backend state for the benchmark flow. Precise RS/ROB/RAT rollback is
    // required before this can safely become es_pipe_flush.
    // CTRL_SAFE_MODE is a debug-only mode for branch/JAL-heavy benchmark bring-up.
    // It prevents fetching/renaming wrong-path instructions, but intentionally
    // trades IPC for simpler control-flow correctness checks.
    assign es_backend_flush = 1'b0;
    wire pc_ce;
    assign pc_ce = fe_ce || es_pc_flush; 
    ...
    wire [`IWIDTH - 1 : 0] im_o_instr_2;
    wire decode_can_load;
`ifdef CTRL_SAFE_MODE
    wire ctrl_safe_decode_clear;
`else
    wire ctrl_safe_decode_clear = 1'b0;
`endif
    ...
    always @(posedge dp_clk, negedge dp_rstn) begin
        if (!dp_rstn) begin
            ...
            if (es_pipe_flush || ctrl_safe_decode_clear) begin
                ds1_rs_o_ce <= 1'b0;
                ds1_rs_o_jal <= 1'b0;
                ds1_rs_o_branch <= 1'b0; 
                ds1_rs_o_regdst <= 1'b0;
    ...
    wire ds_branch_2 = ds2_rs_o_ce && ds2_rs_o_branch;
`ifdef CTRL_SAFE_MODE
    wire ds_jal_req_1 = ds1_rs_o_ce && ds1_rs_o_jal;
    wire ds_jal_req_2 = ds2_rs_o_ce && ds2_rs_o_jal;
`else
...
`endif
    ...
    wire ds_ctrl_1 = ds1_rs_o_ce && (ds1_rs_o_jal || ds1_rs_o_branch);
    wire ds_ctrl_2 = ds2_rs_o_ce && (ds2_rs_o_jal || ds2_rs_o_branch);

`ifdef CTRL_SAFE_MODE
    reg ctrl_serial_pending;
    wire ctrl_safe_resolve;
    wire ctrl_safe_lane2_block = ds_ctrl_1;
`else
    wire ctrl_safe_lane2_block = 1'b0;
`endif
    ...
    wire decode_hold = !dec_pair_done;
`ifdef CTRL_SAFE_MODE
    wire ctrl_accept = (ds_ctrl_1 && dec_consume_1) || (ds_ctrl_2 && dec_consume_2);
    assign ctrl_safe_decode_clear = ctrl_accept;
    wire ctrl_safe_frontend_stall = ctrl_serial_pending || ds_ctrl_1 || ds_ctrl_2;

    always @(posedge dp_clk or negedge dp_rstn) begin
        if (!dp_rstn) begin
            ctrl_serial_pending <= 1'b0;
        end
        else if (es_pc_flush || ctrl_safe_resolve) begin
            ctrl_serial_pending <= 1'b0;
        end
        else if (ctrl_accept) begin
            ctrl_serial_pending <= 1'b1;
        end
    end
`else
    wire ctrl_safe_frontend_stall = 1'b0;
`endif
    wire fe_lane_done_1 = dec_lane_done_1;
    wire fe_lane_done_2 = dec_lane_done_2;
    wire ds_rs_any_valid = ds1_rs_o_ce || ds2_rs_o_ce;
    wire fe_stall = decode_hold || ctrl_safe_frontend_stall;
    assign fe_ce = dp_i_ce && ~fe_stall;
    assign decode_can_load = im_o_ce && ~es_pc_flush && !decode_hold &&
                             !ctrl_safe_frontend_stall;
    wire rs_o_full;
    wire rob_o_full;
...
`ifdef CTRL_SAFE_MODE
    assign ctrl_safe_resolve =
        (es1_o_ce && (is3_jal_1 || (is3_opcode_1 == `BTYPE))) ||
        (es2_o_ce && (is3_jal_2 || (is3_opcode_2 == `BTYPE)));
`endif
