`timescale 1ns/1ps
// Reservation Station:
// - Tracks operand-ready state inside each entry.
// - Selects up to two ready entries in entry order.
// - Outputs selected entries directly to the issue/execute buffer.

//IPC reachs maximum of present design
module reservation_station (
    ...
    always @(*) begin
        ...
        for (idx_c = 0; idx_c < `RS_SIZE; idx_c = idx_c + 1) begin
            ...
            ready_vec[idx_c] = ent_valid[idx_c] &&
                               (!ent_has_rs[idx_c] || ent_rs_ready[idx_c]) &&
                               (!ent_has_rt[idx_c] || ent_rt_ready[idx_c]);
        ...
        if (rs_i_ce) begin
            for (idx_c = 0; idx_c < `RS_SIZE; idx_c = idx_c + 1) begin
                if (!issue2_valid &&
                    (!issue1_valid || (idx_c[rs_idx_w-1:0] != issue1_idx)) &&
        ...
        if (issue1_valid) begin
            rs_o_pc_1 = ent_pc[issue1_idx];
            rs_o_opcode_1 = ent_opcode[issue1_idx];
            rs_o_funct3_1 = ent_funct3[issue1_idx];
            rs_o_funct7_1 = ent_funct7[issue1_idx];
            rs_o_shamt_1 = ent_shamt[issue1_idx];
            rs_o_imm_1 = ent_imm[issue1_idx];
            rs_o_alu_src_1 = ent_alu_src[issue1_idx];
            rs_o_jal_1 = ent_jal[issue1_idx];
            rs_o_memwrite_1 = ent_memwrite[issue1_idx];
            rs_o_memtoreg_1 = ent_memtoreg[issue1_idx];
            rs_o_regwrite_1 = ent_regwrite[issue1_idx];
            rs_o_prd_1 = ent_prd[issue1_idx];
            rs_o_rob_tag_1 = ent_rob_tag[issue1_idx];
            rs_o_sq_idx_1 = ent_sq_idx[issue1_idx];
            rs_o_vrs_1 = ent_vrs[issue1_idx];
            rs_o_vrt_1 = ent_vrt[issue1_idx];
        end

        if (issue2_valid) begin
            rs_o_pc_2 = ent_pc[issue2_idx];
            rs_o_opcode_2 = ent_opcode[issue2_idx];
            rs_o_funct3_2 = ent_funct3[issue2_idx];
            rs_o_funct7_2 = ent_funct7[issue2_idx];
            rs_o_shamt_2 = ent_shamt[issue2_idx];
            rs_o_imm_2 = ent_imm[issue2_idx];
            rs_o_alu_src_2 = ent_alu_src[issue2_idx];
            rs_o_jal_2 = ent_jal[issue2_idx];
            rs_o_memwrite_2 = ent_memwrite[issue2_idx];
            rs_o_memtoreg_2 = ent_memtoreg[issue2_idx];
            rs_o_regwrite_2 = ent_regwrite[issue2_idx];
            rs_o_prd_2 = ent_prd[issue2_idx];
            rs_o_rob_tag_2 = ent_rob_tag[issue2_idx];
            rs_o_sq_idx_2 = ent_sq_idx[issue2_idx];
            rs_o_vrs_2 = ent_vrs[issue2_idx];
            rs_o_vrt_2 = ent_vrt[issue2_idx];
        end
    end
    ...
    always @(posedge rs_clk or negedge rs_rstn) begin
        if (!rs_rstn) begin
            ...
            if (issue1_valid && rs_i_issue_accept_1) begin
                ent_valid[issue1_idx]    <= 1'b0;
                ent_has_rs[issue1_idx]   <= 1'b0;
                ent_has_rt[issue1_idx]   <= 1'b0;
                ent_rs_ready[issue1_idx] <= 1'b0;
                ent_rt_ready[issue1_idx] <= 1'b0;
                ent_sq_idx[issue1_idx]   <= {`ROB_IDX_W{1'b0}};
            end

            if (issue2_valid && rs_i_issue_accept_2) begin
                ent_valid[issue2_idx]    <= 1'b0;
                ent_has_rs[issue2_idx]   <= 1'b0;
                ent_has_rt[issue2_idx]   <= 1'b0;
                ent_rs_ready[issue2_idx] <= 1'b0;
                ent_rt_ready[issue2_idx] <= 1'b0;
                ent_sq_idx[issue2_idx]   <= {`ROB_IDX_W{1'b0}};
            end

            if (rs_i_alloc_valid_1 && !rs_o_stall_1 && free1_valid) begin
                ent_valid[free1_idx]    <= 1'b1;
                ent_pc[free1_idx]       <= rs_i_pc_1;
                ent_opcode[free1_idx]   <= rs_i_opcode_1;
                ent_funct3[free1_idx]   <= rs_i_funct3_1;
                ent_funct7[free1_idx]   <= rs_i_funct7_1;
                ent_shamt[free1_idx]    <= rs_i_shamt_1;
                ent_imm[free1_idx]      <= rs_i_imm_1;
                ent_alu_src[free1_idx]  <= rs_i_alu_src_1;
                ent_jal[free1_idx]      <= rs_i_jal_1;
                ent_memwrite[free1_idx] <= rs_i_memwrite_1;
                ent_memtoreg[free1_idx] <= rs_i_memtoreg_1;
                ent_regwrite[free1_idx] <= rs_i_regwrite_1;
                ent_prs[free1_idx]      <= rs_i_prs_1;
                ent_prt[free1_idx]      <= rs_i_prt_1;
                ent_prd[free1_idx]      <= rs_i_prd_1;
                ent_rob_tag[free1_idx]  <= rs_i_rob_tag_1;
                ent_sq_idx[free1_idx]   <= rs_i_sq_idx_1;
                ent_has_rs[free1_idx]   <= opcode_has_rs(rs_i_opcode_1);
                ent_has_rt[free1_idx]   <= opcode_has_rt(rs_i_opcode_1);
                ent_rs_ready[free1_idx] <= !opcode_has_rs(rs_i_opcode_1) ||
                                           rs_i_prs_ready_1 ||
                                           match_src(rs_i_prs_1);
                ent_rt_ready[free1_idx] <= !opcode_has_rt(rs_i_opcode_1) ||
                                           rs_i_prt_ready_1 ||
                                           match_src(rs_i_prt_1);
                ent_vrs[free1_idx]      <= match_src(rs_i_prs_1) ?
                                           match_src_data(rs_i_prs_1) : rs_i_data_rs_1;
                ent_vrt[free1_idx]      <= match_src(rs_i_prt_1) ?
                                           match_src_data(rs_i_prt_1) : rs_i_data_rt_1;
            end

            if (rs_i_alloc_valid_2 && !rs_o_stall_2) begin
                if (rs_i_alloc_valid_1) begin
                    if (free2_valid) begin
                        ent_valid[free2_idx]    <= 1'b1;
                        ent_pc[free2_idx]       <= rs_i_pc_2;
                        ent_opcode[free2_idx]   <= rs_i_opcode_2;
                        ent_funct3[free2_idx]   <= rs_i_funct3_2;
                        ent_funct7[free2_idx]   <= rs_i_funct7_2;
                        ent_shamt[free2_idx]    <= rs_i_shamt_2;
                        ent_imm[free2_idx]      <= rs_i_imm_2;
                        ent_alu_src[free2_idx]  <= rs_i_alu_src_2;
                        ent_jal[free2_idx]      <= rs_i_jal_2;
                        ent_memwrite[free2_idx] <= rs_i_memwrite_2;
                        ent_memtoreg[free2_idx] <= rs_i_memtoreg_2;
                        ent_regwrite[free2_idx] <= rs_i_regwrite_2;
                        ent_prs[free2_idx]      <= rs_i_prs_2;
                        ent_prt[free2_idx]      <= rs_i_prt_2;
                        ent_prd[free2_idx]      <= rs_i_prd_2;
                        ent_rob_tag[free2_idx]  <= rs_i_rob_tag_2;
                        ent_sq_idx[free2_idx]   <= rs_i_sq_idx_2;
                        ent_has_rs[free2_idx]   <= opcode_has_rs(rs_i_opcode_2);
                        ent_has_rt[free2_idx]   <= opcode_has_rt(rs_i_opcode_2);
                        ent_rs_ready[free2_idx] <= !opcode_has_rs(rs_i_opcode_2) ||
                                                   ((!lane2_dep_rs_on_lane1) &&
                                                    (rs_i_prs_ready_2 || match_src(rs_i_prs_2)));
                        ent_rt_ready[free2_idx] <= !opcode_has_rt(rs_i_opcode_2) ||
                                                   ((!lane2_dep_rt_on_lane1) &&
                                                    (rs_i_prt_ready_2 || match_src(rs_i_prt_2)));
                        ent_vrs[free2_idx]      <= lane2_dep_rs_on_lane1 ? {`DWIDTH{1'b0}} :
                                                   (match_src(rs_i_prs_2) ?
                                                    match_src_data(rs_i_prs_2) : rs_i_data_rs_2);
                        ent_vrt[free2_idx]      <= lane2_dep_rt_on_lane1 ? {`DWIDTH{1'b0}} :
                    ...
                    if (free1_valid) begin
                        ent_valid[free1_idx]    <= 1'b1;
                        ent_pc[free1_idx]       <= rs_i_pc_2;
                        ent_opcode[free1_idx]   <= rs_i_opcode_2;
                        ent_funct3[free1_idx]   <= rs_i_funct3_2;
                        ent_funct7[free1_idx]   <= rs_i_funct7_2;
                        ent_shamt[free1_idx]    <= rs_i_shamt_2;
                        ent_imm[free1_idx]      <= rs_i_imm_2;
                        ent_alu_src[free1_idx]  <= rs_i_alu_src_2;
                        ent_jal[free1_idx]      <= rs_i_jal_2;
                        ent_memwrite[free1_idx] <= rs_i_memwrite_2;
                        ent_memtoreg[free1_idx] <= rs_i_memtoreg_2;
                        ent_regwrite[free1_idx] <= rs_i_regwrite_2;
                        ent_prs[free1_idx]      <= rs_i_prs_2;
                        ent_prt[free1_idx]      <= rs_i_prt_2;
                        ent_prd[free1_idx]      <= rs_i_prd_2;
                        ent_rob_tag[free1_idx]  <= rs_i_rob_tag_2;
                        ent_sq_idx[free1_idx]   <= rs_i_sq_idx_2;
                        ent_has_rs[free1_idx]   <= opcode_has_rs(rs_i_opcode_2);
                        ent_has_rt[free1_idx]   <= opcode_has_rt(rs_i_opcode_2);
                        ent_rs_ready[free1_idx] <= !opcode_has_rs(rs_i_opcode_2) ||
                                                   rs_i_prs_ready_2 ||
                                                   match_src(rs_i_prs_2);
                        ent_rt_ready[free1_idx] <= !opcode_has_rt(rs_i_opcode_2) ||
                                                   rs_i_prt_ready_2 ||
                                                   match_src(rs_i_prt_2);
                        ent_vrs[free1_idx]      <= match_src(rs_i_prs_2) ?
                                                   match_src_data(rs_i_prs_2) : rs_i_data_rs_2;
                        ent_vrt[free1_idx]      <= match_src(rs_i_prt_2) ?
                                                   match_src_data(rs_i_prt_2) : rs_i_data_rt_2;
                    end
                end
            end
...
//     reg issueq_valid_1;
//     reg issueq_valid_2;
//     reg [RS_IDX_W-1:0] issueq_idx_1;
//     reg [RS_IDX_W-1:0] issueq_idx_2;

//     wire issueq_can_load_1 = (!issueq_valid_1) || rs_i_issue_accept_1;
//     wire issueq_can_load_2 = (!issueq_valid_2) || rs_i_issue_accept_2;
...
//                 dbg_pc84_ent_vrt = ent_vrt[idx_dbg];
//                 dbg_pc84_ent_valid = ent_valid[idx_dbg];
//                 dbg_pc84_ent_issued = ent_issued[idx_dbg];
//             end
//         end
//     end
...
//     function match_src_es;
//         input [`RAT_SIZE-1:0] src_tag;
//         begin
//             match_src_es =
//                 (rs_i_es_valid_1 &&
//                  ((rs_i_es_opcode_1 == `RTYPE) || (rs_i_es_opcode_1 == `ITYPE)) &&
//                  (src_tag == rs_i_es_prd_1)) ||
//                 (rs_i_es_valid_2 &&
//                  ((rs_i_es_opcode_2 == `RTYPE) || (rs_i_es_opcode_2 == `ITYPE)) &&
//                  (src_tag == rs_i_es_prd_2));
//         end
//     endfunction
...
//         rs_o_stall_1 = 1'b0;
//         rs_o_stall_2 = 1'b0;
//         rs_o_has_valid = issueq_valid_1 | issueq_valid_2;
//         rs_o_can_alloc_1 = 1'b0;
//         rs_o_can_alloc_2 = 1'b0;
//         rs_o_full = 1'b0;

//         rs_o_issue_valid_1 = issueq_valid_1;
//         rs_o_issue_valid_2 = issueq_valid_2;
...
//             // ready_vec[idx_c] = ent_valid[idx_c] &&
//             //                    !ent_issued[idx_c] &&
//             //                    (!ent_has_rs[idx_c] || ent_rs_ready[idx_c] || match_src_es(ent_prs[idx_c])) &&
//             //                    (!ent_has_rt[idx_c] || ent_rt_ready[idx_c] || match_src_es(ent_prt[idx_c]));
//             ready_vec[idx_c] = ent_valid[idx_c] &&
//                                !ent_issued[idx_c] &&
//                                (!ent_has_rs[idx_c] || ent_rs_ready[idx_c]) &&
//                                (!ent_has_rt[idx_c] || ent_rt_ready[idx_c]);
...
//         if (rs_i_ce && issueq_can_load_1) begin
//             for (idx_c = 0; idx_c < `RS_SIZE; idx_c = idx_c + 1) begin
//                 if (!issue1_valid && ready_vec[idx_c]) begin
//                     issue1_valid = 1'b1;
//                     issue1_idx = idx_c[RS_IDX_W-1:0];
...
//         if (rs_i_ce && issueq_can_load_2) begin
//             for (idx_c = 0; idx_c < `RS_SIZE; idx_c = idx_c + 1) begin
//                 if (!issue2_valid &&
//                     (!issue1_valid || (idx_c[RS_IDX_W-1:0] != issue1_idx)) &&
//                     ready_vec[idx_c]) begin
...
//         if (issueq_valid_1) begin
//             rs_o_pc_1 = ent_pc[issueq_idx_1];
//             rs_o_opcode_1 = ent_opcode[issueq_idx_1];
//             rs_o_funct3_1 = ent_funct3[issueq_idx_1];
//             rs_o_funct7_1 = ent_funct7[issueq_idx_1];
//             rs_o_shamt_1 = ent_shamt[issueq_idx_1];
//             rs_o_imm_1 = ent_imm[issueq_idx_1];
//             rs_o_alu_src_1 = ent_alu_src[issueq_idx_1];
//             rs_o_jal_1 = ent_jal[issueq_idx_1];
//             rs_o_memwrite_1 = ent_memwrite[issueq_idx_1];
//             rs_o_memtoreg_1 = ent_memtoreg[issueq_idx_1];
//             rs_o_regwrite_1 = ent_regwrite[issueq_idx_1];
//             rs_o_prd_1 = ent_prd[issueq_idx_1];
//             rs_o_rob_tag_1 = ent_rob_tag[issueq_idx_1];
//             rs_o_sq_idx_1 = ent_sq_idx[issueq_idx_1];
//             rs_o_vrs_1 = ent_vrs[issueq_idx_1];
//             rs_o_vrt_1 = ent_vrt[issueq_idx_1];
//         end

//         if (issueq_valid_2) begin
//             rs_o_pc_2 = ent_pc[issueq_idx_2];
//             rs_o_opcode_2 = ent_opcode[issueq_idx_2];
//             rs_o_funct3_2 = ent_funct3[issueq_idx_2];
//             rs_o_funct7_2 = ent_funct7[issueq_idx_2];
//             rs_o_shamt_2 = ent_shamt[issueq_idx_2];
//             rs_o_imm_2 = ent_imm[issueq_idx_2];
//             rs_o_alu_src_2 = ent_alu_src[issueq_idx_2];
//             rs_o_jal_2 = ent_jal[issueq_idx_2];
//             rs_o_memwrite_2 = ent_memwrite[issueq_idx_2];
//             rs_o_memtoreg_2 = ent_memtoreg[issueq_idx_2];
//             rs_o_regwrite_2 = ent_regwrite[issueq_idx_2];
//             rs_o_prd_2 = ent_prd[issueq_idx_2];
//             rs_o_rob_tag_2 = ent_rob_tag[issueq_idx_2];
//             rs_o_sq_idx_2 = ent_sq_idx[issueq_idx_2];
//             rs_o_vrs_2 = ent_vrs[issueq_idx_2];
//             rs_o_vrt_2 = ent_vrt[issueq_idx_2];
//         end
//     end
...
//             issueq_valid_1 <= 1'b0;
//             issueq_valid_2 <= 1'b0;
//             issueq_idx_1 <= {RS_IDX_W{1'b0}};
//             issueq_idx_2 <= {RS_IDX_W{1'b0}};
...
//                 end
//             end
//             if (issueq_valid_1 && rs_i_issue_accept_1) begin
//                 ent_valid[issueq_idx_1]    <= 1'b0;
//                 ent_has_rs[issueq_idx_1]   <= 1'b0;
//                 ent_has_rt[issueq_idx_1]   <= 1'b0;
//                 ent_rs_ready[issueq_idx_1] <= 1'b0;
//                 ent_rt_ready[issueq_idx_1] <= 1'b0;
//                 ent_issued[issueq_idx_1]   <= 1'b0;
//                 ent_sq_idx[issueq_idx_1]   <= {`ROB_IDX_W{1'b0}};
//                 issueq_valid_1 <= 1'b0;
//             end

//             if (issueq_valid_2 && rs_i_issue_accept_2) begin
//                 ent_valid[issueq_idx_2]    <= 1'b0;
//                 ent_has_rs[issueq_idx_2]   <= 1'b0;
//                 ent_has_rt[issueq_idx_2]   <= 1'b0;
//                 ent_rs_ready[issueq_idx_2] <= 1'b0;
//                 ent_rt_ready[issueq_idx_2] <= 1'b0;
//                 ent_issued[issueq_idx_2]   <= 1'b0;
//                 ent_sq_idx[issueq_idx_2]   <= {`ROB_IDX_W{1'b0}};
//                 issueq_valid_2 <= 1'b0;
//             end

//             if (issueq_can_load_1 && issue1_valid) begin
//                 issueq_valid_1 <= 1'b1;
//                 issueq_idx_1 <= issue1_idx;
//                 ent_issued[issue1_idx] <= 1'b1;
//             end

//             if (issueq_can_load_2 && issue2_valid) begin
//                 issueq_valid_2 <= 1'b1;
//                 issueq_idx_2 <= issue2_idx;
//                 ent_issued[issue2_idx] <= 1'b1;
//             end
