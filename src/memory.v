...
`ifdef BEEBS_DATA_INIT
    localparam DATA_MEM_DEPTH = `MEM_DEPTH;
`else
    localparam DATA_MEM_DEPTH = `AWIDTH_MEM;
`endif
    localparam MEM_IDX_W = $clog2(DATA_MEM_DEPTH);
    ...
    integer i;
    reg [`DWIDTH - 1 : 0] data_mem [0 : DATA_MEM_DEPTH - 1];

    // ============================================================
    // Debug internal memory words for waveform only
    // Add dbg_data_mem[*] to waveform to observe data_mem[*]
    // ============================================================
    wire [`DWIDTH - 1 : 0] dbg_data_mem [0 : DATA_MEM_DEPTH - 1];

    genvar dbg_i;
    generate
        for (dbg_i = 0; dbg_i < DATA_MEM_DEPTH; dbg_i = dbg_i + 1) begin : GEN_DBG_DATA_MEM
